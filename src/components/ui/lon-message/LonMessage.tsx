...
import "./LonMessage.css";

export type LonMessageType = "success" | "info" | "warning" | "error" | "loading";

export type LonMessageOptions = {
  type?: LonMessageType;
  content: ReactNode;
  duration?: number;
  closable?: boolean;
...
export type LonMessageApi = {
  open: (options: LonMessageOptions) => string;
  success: (content: ReactNode, options?: Omit<LonMessageOptions, "content" | "type">) => string;
  info: (content: ReactNode, options?: Omit<LonMessageOptions, "content" | "type">) => string;
  warning: (content: ReactNode, options?: Omit<LonMessageOptions, "content" | "type">) => string;
  error: (content: ReactNode, options?: Omit<LonMessageOptions, "content" | "type">) => string;
  loading: (content: ReactNode, options?: Omit<LonMessageOptions, "content" | "type">) => string;
  destroy: (id?: string) => void;
};

type InternalMessage = Required<Pick<LonMessageOptions, "type" | "duration" | "closable">> & {
  id: string;
  content: ReactNode;
  phase: "open" | "closing" | "collapsing";
};

const MessageContext = createContext<LonMessageApi | null>(null);
const MESSAGE_EXIT_DURATION = 160;
const MESSAGE_COLLAPSE_DURATION = 160;
const MESSAGE_MAX_COUNT = 4;

function getMessageDefaults(type: LonMessageType, duration?: number, closable?: boolean) {
  return {
...
function MessageIcon({ type }: { type: LonMessageType }) {
  if (type === "success") {
  ...
  if (type === "loading") {
    return <LoaderCircle className="lon-message-spinner" size={17} strokeWidth={2.2} />;
  }
...
function MessageItem({ message, onClose }: { message: InternalMessage; onClose: (id: string) => void }) {
  ...
  return (
    ...
    >
      <span className="lon-message-icon" aria-hidden="true">
        <MessageIcon type={message.type} />
      </span>
      <span className="lon-message-content">{message.content}</span>
      {message.closable ? (
        <button className="lon-message-close" type="button" aria-label="关闭提示" onClick={() => onClose(message.id)}>
...
export function LonMessageProvider({ children }: { children: ReactNode }) {
  ...
  const open = useCallback((options: LonMessageOptions) => {
  ...
  const api = useMemo<LonMessageApi>(
    () => ({
      open,
  ...
  return (
    <MessageContext.Provider value={api}>
      {children}
      <div className="lon-message-viewport" aria-label="全局提示">
...
export function useLonMessage() {
  const api = useContext(MessageContext);

  if (!api) {
    throw new Error("useLonMessage must be used within LonMessageProvider");
  }
